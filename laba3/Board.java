...
public class Board {
    ...
    public Iterable<Board> neighbors() {  // все соседние позиции
        ...
        // если соседнего нет (0 может быть с краю), change_2_fields() вернет null
        Set<Board> boardList = new HashSet<Board>();
        boardList.add(change_2_fields(getNewBlock(), zeroX, zeroY, zeroX, zeroY + 1));
        boardList.add(change_2_fields(getNewBlock(), zeroX, zeroY, zeroX, zeroY - 1));
        boardList.add(change_2_fields(getNewBlock(), zeroX, zeroY, zeroX - 1, zeroY));
        boardList.add(change_2_fields(getNewBlock(), zeroX, zeroY, zeroX + 1, zeroY));
    ...
    private Board change_2_fields(int[][] blocks2, int x1, int y1, int x2, int y2) {  //меняет 2 соседних поля
